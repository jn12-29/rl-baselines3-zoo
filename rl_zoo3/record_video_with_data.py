...
from rl_zoo3.representation_extractor import RepresentationExtractor
...
if __name__ == "__main__":
    ...
    try:
        for step in range(video_length):
            ...
            if activations:
                activations = {k: v.cpu().numpy() if hasattr(v, "cpu") else v for k, v in activations.items()}
                all_activations.append(activations)

            data_recorder.record_step(obs=obs, action=action, reward=0, done=False, env=raw_env, activations=activations)
